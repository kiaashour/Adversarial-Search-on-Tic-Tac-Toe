...
#Implement the Alpha-Beta pruning algorithm for tic-tac-toe on a (m,n,k)-board
class Game():
    def __init__(self, m, n, k, automatic=True, display=True):
        """ Initialize the a game of tic-tac-toe on an m x n board
            ...
            or column or diagonal. This uses the Alpha-Beta pruning algorithm
        ...
        self.all_possible_moves = {(i, j) for i in range(m) for j in range(n)}

        #Intiialise game statistics
        self.times = []
    ...
    def max(self, alpha=-math.inf, beta=math.inf):
        """Compute the max value of the game. 

        Args:
            alpha (int, optional): The alpha value. Defaults to -math.inf.
            beta (int, optional): The beta value. Defaults to math.inf.

        Returns:    
            (int, (i, j)): The value of the game and the best move
        """
        ...
        for i,j in possible_moves:        
            ...
            if self.is_valid(i, j):        
                ...
                self.last_move = (i, j)
                min_v, best_move_min = self.min(alpha, beta)

                #If the min value is greater than the max value, then update the max value
                if v >= beta:
                    self.board[i][j] = 0
    ...
    def min(self, alpha=-math.inf, beta=math.inf):
        ...
        for i,j in possible_moves:
            ...
            if self.is_valid(i, j):
                ...
                temporary_last_move = self.last_move
                ...
                #If the max value is less than the min value, then update the min value
                if v <= alpha:
                    self.board[i][j] = 0
                    self.moves[-1].remove((i, j))
                    self.last_move = temporary_last_move
                    return v, best_move
                beta = min(beta, v)
    ...
    def is_terminal(self, last_player):
        ...
        for direction in possible_directions:
            count = 0
            #Check the previous action moving to the right direction
            for step in range(1, self.k):
                ...
                if next_action in prev_actions:
                    ...
                    break
            #Check the previous action moving to the left direction
            for step in range(1, self.k):
                next_action = direction(prev_action, -step)
                if next_action in prev_actions:
                    count += 1
    ...
    def receive_move(self, player):
        ...
        Args:
            player (int): The player to move (1 for MAX, -1 for MIN)

        Returns:
            (i, j): The move
        """
        #If the player is 1, then take the input and recommend the best move
        time1 = time.time()
        if player == 1:
            ...
            #If not automatic, then player 1 takes the input
                ...
                #Recommend best move and take input
        ...
        #If the player is -1, then take the best move
        else:
